...
tag_speaker_input_form <- function(ns_prefix, ind_evaluation_date = TRUE) {
  ...
  if (ind_evaluation_date) {
    tags_to_return <- tagList(
      ...
      dateInput(paste0(ns_prefix, "-date_of_birth"), "Date of Birth *", min = as.Date("1900-01-01"), max = Sys.Date()),
      dateInput(paste0(ns_prefix, "-date_of_evaluation"), "Date of Evaluation *", min = Sys.Date() - 365*10, max = Sys.Date()),
